...
@as_string.register
def as_string_named(obj: Named) -> str:
    if obj.name is None:
        return "<anonymous>"
    if obj.namespace:
...
def _(obj: NamedExpression) -> str:
    if obj.expression is None:
        return as_string_named(obj)
    return as_string(obj.expression)
...
def _(obj: Expression) -> str:
    func = obj.func
    if func.fmt is None:
        plain_args = tuple(map(as_string, obj.args)) + tuple(
            f"{k}={as_string(v)}" for k, v in obj.kwargs.items()
        )
        return f"{as_string(func)}({', '.join((v for v in plain_args))})"
    ...
    if isinstance(func, BinaryOperator):
        ...
        return func.fmt.format(
            *map(as_string, obj.args),
            **{k: as_string(v) for k, v in obj.kwargs.items()},
        )
